...
import Control.Effect.Error (catchError, throwError)
...
import Data.RRBVector qualified as V
  ...
  let unresolved =
        foldl'
          ( \acc e -> case e of
              ...
              _ -> error "Internal error: Unexpected entry"
          )
          []
          finalEntries
    ...
    Scopes
      ...
      ( adjust'
          ...
          ( fmap \scope ->
              let (before, after) =
                    splitAt
                      ( fromMaybe (error "Internal error: Ex not found in context")
                          $ findEVarIndexScope beforeSub scope
                      )
                      scope
...
withRewr rewr cont = do
  ...
  cont <* modify \(Scopes bs es rs) ->
    Scopes
      bs
      (adjust' i (fmap $ maybe (error "Internal error: Scope disappeared") fst . viewr) es)
      (maybe (error "Internal error: Rewrite disappeared") fst $ viewr rs)
...
logAndRunInfer :: forall a. ((TermF Term, (Epoch, InferMode a)) -> ScopesM a) -> Term -> InferMode a -> ScopesM a
logAndRunInfer f t mode =
  let act = getEpoch >>= \e -> f (unTerm t, (e, mode))
   in case t of
        Term Block {} -> act -- No logging to reduce noise
        _ ->
          let scope x = stackScope @a \p -> "<" <> group (p t) <> "> : " <> x p
           in case mode of
                Infer -> scope (\_ -> "_") do
                  res <- act
...
inferApp ie f0 args0 (em, mode) = case f0 of
  ...
  where
      ...
      foldr
        ...
        ( \(args, outT0) -> do
            outT <- dyn outT0
            let checkArgs = for_ args \(q, arg, argTy) -> infer (restrictEra q ie) arg . Check =<< fetchT argTy
            case mode of
...
-- -}
rowGet :: forall a. ((Term -> ScopesM Term) -> a -> ScopesM a) -> Term -> (Term -> ScopesM a) -> Term -> Term -> ScopesM (LookupRes a)
              ...
              (fDyn e -> Concat l (FRow r)) -> do
                ...
                case inL of
                  LookupMissing visited1 -> do
                    ...
                    let select f = normalize $ Term $ App (Term $ App (Term $ Builtin f) $ Term $ ListLit $ Vector' visited1') record
...
infer :: forall a. IsErased -> Term -> InferMode a -> ScopesM a
infer ie = logAndRunInfer $ \case
  ...
  (Block (BlockLet nameM q tyM val into), mode0) -> do
    let varName = either (\(OpaqueId n _) -> Just n) id nameM
    bty <- stackScope (\_ -> "let" <+> pQuant q <> maybe "_" pIdent varName)
      $ case tyM of
        ...
        Just ty -> do
          ...
          pure ty'
    bval <- normalize val
    (ty', val') <- case nameM of
      Left oid -> do
        freeVarM <-
          runError @Int (pure . Just) (\() -> pure Nothing) $ runReader @Int 0 $ bval & fix \rec ->
            unTerm >>> \case
              Var x -> do
                locals <- ask
        ...
        case freeVarM of
          Just x -> stackLog \p -> "Opaque" <+> pOpaqueId oid <+> "mentions free variable" <+> p (Term $ Var x)
          Nothing -> pure ()
        lift $ lift $ registerOpaque oid bty
        let modty =
              Term
                $ Refine
                $ RefinePostTy bty (regIdent "un")
                $ Lambda
                $ TBuiltin Eq
                `TApp` TBuiltin (OpaqueVal oid)
                `TApp` bval
        pure (modty, TBuiltin $ OpaqueVal oid)
      Right _ -> pure (bty, bval)
    let fInto :: ((Term -> ScopesM Term) -> a -> ScopesM a) -> InferMode a -> ScopesM a
        fInto mapTerm mode =
          mapTerm (\t -> pure $ fromMaybe (error "Internal error: let var leaked") $ nestedBy' 0 t (-1))
            =<< withBinding (q, varName, Just val', ty') (infer ie (unLambda into) mode)
  ...
  (App (unTerm -> App (unTerm -> Builtin RecordGet) tag) record, mode) -> do
    ...
    let mapTerm :: (Term -> ScopesM Term) -> a -> ScopesM a
        cont :: Term -> ScopesM a
  ...
  (ExVar (scopeid, subid), m0) -> do
    ty <- runEmpty (error "Internal error: Ex not found in scope") pure do
      Just (_, Right ty) <- findEVar (scopeid, subid) <$> get @Scopes
      pure ty
    let againstMode :: ScopesM a
        againstMode = case m0 of
          (_, Infer) -> nestBinding scopeid ty
          (ce0, Check cty0) -> do
            subtype ty cty0 `catchError` \(_ :: Doc AnsiStyle) -> do
              -- Fail, trying to constraint Ex.
              subtype cty0 ty
              cty <- fetchT (Dyn ce0 cty0)
              -- Hack: the proper way is to introduce an existential `a' : cty`, but we're skipping this in favour of
              -- restricting `a` directly.
              Just (i, _) <- findEVar (scopeid, subid) <$> get @Scopes
              modify \(Scopes bs0 exs0 rs0) ->
                Scopes
                  bs0
                  ( V.adjust
                      subid
                      ( \(resolvedE, scopes) ->
                          ( resolvedE {- not resolving ⇒ no update -},
                            V.adjust
                              i
                              ( \case
                                  EVar _ (Right _olty) -> EVar subid $ Right cty
                                  _ -> error "impossible"
                              )
                              scopes
                          )
...
typOfBuiltin opaques = \case
  ...
  Never -> [termQQ| Type^ 0 |]
  OpaqueVal x -> fromMaybe (error "Internal error: opaque not found") $ HM.lookup x opaques
  ...
  KolId -> [termQQ| Any |]
  KolLocEventId -> [termQQ| Fun (EventTypeId) -> Type^ 0 |]
  KolMkEventType → [termQQ| Fun ({(.key = Tag | .group = Type^ 0 | .body = Type^ 0)}) -> EventTypeId |]
  KolMkGear -> [termQQ| Fun {Out} {Cache : Type^ 0} ({( .primary = {( .type = EventTypeId | .group = Any )} | .initial_cache = Cache | .step = Fun (Cache) (Primary) (Secondary) -> {( .cache = Cache | .out = Out )} )}) -> Gear Out |]
  KolMkQuery -> [termQQ| Fun {Body} -> Query Body |]
  KolMkStateGraph -> [termQQ| Any |]
  KolUserId -> [termQQ| Any |]
  KolUnEventType -> [termQQ| Fun (EventTypeId) -> {(.key = Tag | .group = Type^ 0 | .body = Type^ 0)} |]
  KolQuery -> [termQQ| Fun (EventTypeId) -> Type^ 0 |]
  KolQueryDelta -> [termQQ| Fun {type : EventTypeId} (Query type) (Primary) -> {( .query = Query type | .delta = {( .removed = List (LocEventId type) | .added = List (LocEventId type) )} )} |]
  KolResolveEvent → [termQQ| Fun {type : EventTypeId} (LocEventId type) -> {( .sender = SenderId | .body = (un_event_type type).body )} |]
  KolResolveData → [termQQ| Any |]
  KolTimestamp -> [termQQ| Type^ 0 |]
  -- StateGraph builtins
  ...
  KolUserEq -> [termQQ| Any |]
  KolMkAnchorAgg -> [termQQ| Any |]
  KolAnchorAggApply -> [termQQ| Any |]
  KolMkTextAgg -> [termQQ| Any |]
  KolTextAggApply -> [termQQ| Any |]
  KolTextAggMerge -> [termQQ| Any |]
  KolSecondaryGet -> [termQQ| Any |]
  KolLoopIter -> [termQQ| Any |]
  KolIterList -> [termQQ| Any |]
  KolSenderId -> [termQQ| Type^ 0 |]
...
instMeta = (\f a b -> stackScope (\_ -> "instMeta") $ f a b) \(scope1, sub1) ->
  let getOrd (scopeI, subI) = do
        Scopes _ exs _ <- get @Scopes
        let exsLen = length exs
        pure $ (scopeI,) $ fromMaybe (error $ "Internal error: Ex not found in scope: scopeI=" <> show scopeI <> ", subI=" <> show subI <> ", exsLen=" <> show exsLen) do
          (_, scope) <- exs !? scopeI
          findIndexL
            ( \case
                EMarker -> False
          ...
          ExVar (scope2, sub2) -> do
            ...
            if ord2 <= ord1
              then pure $ Term $ ExVar (scope2, sub2)
                ...
                u <- lift fresh
                Scopes _ exs _ <- get @Scopes
                let t2 = fromMaybe (error $ "Internal error: Ex not found in scope (instMeta'): ord2=" <> show ord2 <> ", exsLen=" <> show (length exs)) do
...
checkModule (Module ms) = do
  ...
  pure $ maybe (error "Internal error: module must be non-empty") (snd . snd) $ viewr imports
...
testBuild :: OsPath -> IO BSL.ByteString
testBuild path' = do
