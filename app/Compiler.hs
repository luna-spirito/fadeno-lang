...
import Parser (BlockF (..), BuiltinT (..), FieldsK (..), Ident (..), Lambda (..), Module (..), Quant (..), RefineK (..), Term (..), TermF (..), Vector' (..), builtinsList, identOfBuiltin, nestedByP, regIdent, splitAt3, traverseTermF, pattern TApp, pattern TBuiltin)
...
lambdaToClosure n orig =
  let captures = listCaptures n orig
      realCaptures = reverse $ imap (\i () -> i `IS.member` captures) (replicate (maybe 0 fst $ IS.maxView captures) ())
   in (captures, Lambda $ erase (realCaptures <> replicate (fromIntegral n) True) $ unLambda orig)
...
registry :: forall r sig m. (Has (State (HashMap r Word64)) sig m, Hashable r) => r -> m Word64
registry val = do
...
compile' =
  unTerm >>> \case
    ...
    Builtin KolMkQuery -> pure $ CConst $ VKolQuery 0
    ...
    Lam QNorm _ body -> do
      ...
      let captures' = CGen . icopy <$> IS.toDescList captures
