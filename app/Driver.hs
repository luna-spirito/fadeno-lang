...
import Normalize (EqRes (..), applyLambda, applyLambdaRefineGetSkip, fDyn, fetchLambda, fetchT, normalize, normalize', numDecDispatch, termQQ, traverseIsEq, withBinding, withMarked, findEVarIndexScope, findEVar)
...
import Prettyprinter (annotate, defaultLayoutOptions, group, layoutSmart, line, list, pretty, (<+>), Doc)
import Prettyprinter.Render.Terminal (Color (..), color, AnsiStyle)
      ...
      againstMode = case m0 of
        ...
        (ce0, Check cty0) → do
          subtype ty cty0 `catchError` \(_ :: Doc AnsiStyle) → do
            ...
            modify \(Scopes bs0 exs0 rs0) → Scopes
              bs0
              (V.adjust subid (\(resolvedE, scopes) → (resolvedE {- not resolving ⇒ no update -}, V.adjust i (\case
        ...
        ExVar (scope2, sub2) → do
          ...
          if ord2 <= ord1
            then pure $ Term $ ExVar (scope2, sub2)
              ...
              Scopes _ exs _ ← get @Scopes
              let
                t2 = fromMaybe (error $ "Internal error: Ex not found in scope (instMeta'): ord2=" <> show ord2 <> ", exsLen=" <> show (length exs)) do
