...
pTerm (oldPrec, vars) =
  withPrec oldPrec . \case
    ...
    Concat a b →
      ( 5
      , case b of
          Left (n, b') → pIdent n <+> ":" <+> pTerm (6, vars) a <+> "\\/" <+> pTerm (5, (QNorm, n) <| vars) (unLambda b')
    ...
    AppErased lam arg → (6, pTerm (6, vars) lam <+> "@" <> pTerm (7, vars) arg)
    ...
    Var x → (7, maybe ("#" <> pretty x) (\(q, i) → pQuant q <> pIdent i) $ vars !? x)
